...
- **Memory quality improvements**: Agent-driven retrieval working. Remaining: empty transitional text on some turns (model behavior), legacy garbage memories need cleanup, enrichment causes occasional duplicate questions. **Critical**: Mem0 extraction LLM (nemotron-3-nano) hallucinates facts from generic content; assistant messages fed to extraction create feedback loop; UPDATE overwrites original source — see memory file for full 10-issue list
