...
- **Structured inputs**: `StructuredInputView` renders interactive questions (multiple_choice, scale, yes_no, multi_select). Scale type uses `ScaleSlider` — gesture-driven discrete slider with color gradient track, ghost thumb affordance, haptic intensity gradient. Interactive when unanswered on latest assistant message; completed state when answered. `onStructuredResponse` callback flows through `ConversationView` → `useConversation.doSend`
...
- **TestFlight gap fixes**: PR #43 (in review). Account deletion (`DELETE /auth/account` with cascade + Mem0 + Supabase cleanup, best-effort), ToS/Privacy links (`salk.health`), constant medical disclaimer (`constants/disclaimer.ts` — NOT dynamic SSE-cached state), 401 re-auth (coalesced token refresh + single retry in `request()`), `expo-splash-screen` (prevents auth loading flash), offline banner (`@react-native-community/netinfo` with 1s debounce, `useNetworkStatus` hook), `ErrorBoundary` class component wrapping root Stack, version from `expo-constants`, Gemini 120s `asyncio.wait_for` timeout, `/health` DB connectivity check, `ModeToggle` animated label (text reveals on focus via `useFocusEffect`, collapses after 1s), profile wizard step labels, welcome copy clarification
...
- **Disclaimer persistence**: ✅ Resolved (PR #43). Replaced dynamic SSE-cached state with a constant `MEDICAL_DISCLAIMER` in `constants/disclaimer.ts` rendered directly in `ConversationView`. The disclaimer text is static — caching was unnecessary
...
- Medical disclaimer is a CONSTANT (`constants/disclaimer.ts`), not dynamic state. Never cache/persist it via SSE events — just render the constant when messages exist
- `ErrorBoundary` wraps root Stack in `_layout.tsx` — class component using `Appearance` API directly (not `useColors()` hook). Shows branded error screen with AnimatedSalkIcon
- `useNetworkStatus` hook wraps `@react-native-community/netinfo` with 1s debounce. Requires native rebuild (`npx expo run:ios`) after install
- Account deletion: `DELETE /api/v1/auth/account` — cascades all data, best-effort Mem0 + Supabase cleanup. Frontend disables button during delete, shows error alert on failure
- 401 retry: `request()` in `api.ts` auto-refreshes expired tokens with coalesced promise. Single retry, sign-out on double failure. Does NOT cover SSE streams (`streamMultipartRequest`)
- `ModeToggle` uses `useFocusEffect` (not `useEffect`) to animate label reveal on every screen focus — same Drawer pitfall as other focus-dependent animations
